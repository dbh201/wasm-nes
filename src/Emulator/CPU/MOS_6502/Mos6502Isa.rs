...
impl Mos6502Isa for Mos6502<'_> {
    ...
    fn _decode_imm(&mut self) -> u8 {
        let t = self.pc;
        self.pc += 1;
        self.getmem(t).unwrap()
            
    }

    fn _decode_zp(&mut self) -> u8 {
        let addr = self._addr_zp();
        self.getmem(addr).unwrap()
    }
    fn _addr_zp(&mut self) -> u16 {
        let t = self.pc;
        self.pc += 1;
    ...
    fn _addr_abs_x(&mut self, add_cycle_on_page_boundary: bool) -> u16 {
        let addr = self._fetch_u16(self.pc).unwrap();
        self.pc += 2;
    ...
    fn _decode_ind_x(&mut self) -> u8 {
        ...
        let ret = self.getmem(addr).unwrap();
        ret
    }
    #[allow(arithmetic_overflow)]
    fn _addr_ind_x(&mut self) -> u16 {
        let t = self.pc;
        self.pc += 1;
        let param = self.getmem(t).unwrap() as u16;

        // Wrap around for x-indexed zero page? Is this a bug, or is it a feature?
        let addr = (self.getmem((param + (self.x as u16)) % 256).unwrap() as u16)
            + ((self.getmem((param + (self.x as u16) + 1) % 256).unwrap() as u16)<<8);
        addr
    }
    ...
    fn _addr_ind_y(&mut self, add_cycle_on_page_boundary: bool) -> u16 {
        ...
        let addr = (self.getmem((param) % 256).unwrap() as u16)
            + ((self.getmem((param + 1) % 256).unwrap() as u16)<<8);
        if add_cycle_on_page_boundary {
            self._check_page_boundary(addr, self.y);
        }
    ...
    fn _pop(&mut self) -> u8 {
        self.sp += 1;
        self.getmem(0x100 + self.sp as u16).unwrap()
    }
    fn _rol(&mut self, addr: u16) {
        let mut val: u8 = self.getmem(addr).unwrap();
    ...
    fn _asl(&mut self, addr: u16) {
        let mut i = self.getmem(addr).unwrap();
    ...
    fn _dec(&mut self, addr: u16) {
        let val = self.getmem(addr).unwrap() as u16 + 255;
    ...
    fn _inc(&mut self, addr: u16) {
        let val = self.getmem(addr).unwrap() as u16 + 1;
        let val = (val % 256) as u8;
        self._set_z_n(val);
        self.setmem(addr, val);
    }
    ...
    fn _lsr(&mut self, addr: u16) {
        let mut val = self.getmem(addr).unwrap();
    ...
    fn brk(&mut self) {
        ...
        self._psh(self.ps);
        self.pc = self._fetch_u16(0xFFFE).unwrap();
